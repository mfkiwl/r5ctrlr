...
int InitAD3552(void)
  ...
  for(i=0; i<NUM_DACS; i++)
    ...
    *(DAC_BADDR[i]+DAC_CTRL_WORD) = SPI_CLK_NO_DIVIDER;
    ...
    // remove reset
    theerr=WriteDacRegister(i,AD3552_INTERFACE_CONFIG_A, 0x10);
    if(theerr!=DAC_SPI_NOERR) return XST_FAILURE;
    
    // set 10V fullscale
    theerr=WriteDacRegister(i,AD3552_CH0_CH1_OUTPUT_RANGE, 0x44);
    if(theerr!=DAC_SPI_NOERR) return XST_FAILURE;
    
    // 6-byte loopback on streaming
    theerr=WriteDacRegister(i,AD3552_STREAM_MODE, 0x06);
    if(theerr!=DAC_SPI_NOERR) return XST_FAILURE;
